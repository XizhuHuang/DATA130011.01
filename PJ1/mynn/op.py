...
class L2Regularization(Layer):
    ...
    def forward(self, model):
        """
        Computes the L2 regularization loss for all optimizable layers in the model.
        model: the neural network model containing optimizable layers
        """
        self.model = model
        self.total_loss = 0
        ...
        for layer in model.layers:
            if layer.optimizable and hasattr(layer, 'params'):
                # Sum the squared weights for each optimizable layer
                for param_name, param_value in layer.params.items():
                    if param_name.startswith('W'):  # Only regularize weight matrices, not biases
                        self.total_loss += 0.5 * self.lambda_ * np.sum(param_value ** 2)
        
        return self.total_loss
    
    def backward(self):
        """
        Computes the gradient of the L2 regularization term and adds it to the existing gradients.
        """
        # Iterate through all layers in the model
        for layer in self.model.layers:
            if layer.optimizable and hasattr(layer, 'params') and hasattr(layer, 'grads'):
                # Add L2 gradient to existing gradients for each optimizable layer
                for param_name, param_value in layer.params.items():
                    if param_name.startswith('W'):  # Only regularize weight matrices, not biases
                        if layer.grads[param_name] is not None:
                            layer.grads[param_name] += self.lambda_ * param_value
                        else:
                            layer.grads[param_name] = self.lambda_ * param_value
...
class Dropout(Layer):
    ...
    def __call__(self, X):  # 新增__call__方法
        return self.forward(X)

    def forward(self, X):
        if self.training:
            self.mask = (np.random.rand(*X.shape) < self.p) / self.p  # 反向传播时除以p保持期望
            return X * self.mask
        else:
            return X  # 测试时不应用Dropout

    def backward(self, grad):
        if self.training:
            # 确保 mask 的维度与 grad 匹配
            if self.mask.shape != grad.shape:
                raise ValueError(f"Mask shape {self.mask.shape} does not match grad shape {grad.shape}")
            return grad * self.mask  # 仅保留激活部分的梯度
        else:
            return grad